...
export default function IncomeScreen({ navigation }: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await incomeMutation.mutate(payload);
      setModalVisible(false);
      resetForm();
      await fetchIncomes();
      Alert.alert('Renda adicionada', 'O lançamento foi salvo com sucesso.');
    } catch {
      Alert.alert('Não foi possível salvar', 'Confira sua conexão e tente novamente. A mesma intenção original será reutilizada enquanto este formulário permanecer aberto.');
    } finally {
      setSaving(false);
    }
  };
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalOverlay}>
          <View accessibilityViewIsModal style={styles.modalContainer}>
            <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
              ...
              <View accessibilityRole="radiogroup" style={styles.typeRow}>
                {(['salary', 'extra', 'adjustment'] as Income['type'][]).map((candidate) => (
                  <TouchableOpacity
                    ...
                    accessibilityState={{ selected: type === candidate, disabled: saving }}
                    disabled={saving}
                    style={[
                      styles.typeBtn,
              ...
              <TextInput accessibilityLabel="Título da renda" accessibilityLabelledBy="income-title-label" editable={!saving} style={styles.input} placeholder="Ex.: Salário, iFood" value={title} onChangeText={setTitle} maxLength={100} returnKeyType="next" />

              <Text nativeID="income-amount-label" style={styles.label}>Valor</Text>
              <View style={styles.inputWrapper}>
                <Text style={styles.currencyPrefix}>R$</Text>
                <TextInput accessibilityLabel="Valor da renda em reais" accessibilityLabelledBy="income-amount-label" editable={!saving} style={styles.inputAmount} placeholder="0,00" keyboardType="numeric" value={amount} onChangeText={handleAmountChange} returnKeyType="next" />
              </View>

              <Text nativeID="income-description-label" style={styles.label}>Descrição opcional</Text>
              <TextInput accessibilityLabel="Descrição opcional da renda" accessibilityLabelledBy="income-description-label" editable={!saving} style={[styles.input, styles.descriptionInput]} placeholder="Ex.: Pagamento semanal" value={description} onChangeText={setDescription} maxLength={255} multiline textAlignVertical="top" />
