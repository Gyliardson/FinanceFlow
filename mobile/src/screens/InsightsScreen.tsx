  ...
  if (loadError || !data) {
    return (
      <View style={styles.stateContainer} accessibilityLiveRegion="assertive">
        ...
        <TouchableOpacity style={styles.retryButton} onPress={() => { setLoading(true); fetchInsights(); }} accessibilityRole="button" accessibilityLabel="Tentar carregar os insights novamente">
      ...
      <ScrollView style={styles.scrollContainer} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.section}>
          <View style={styles.rowBetween}>
            ...
            <TouchableOpacity style={styles.addReserveBtn} onPress={() => setModalVisible(true)} accessibilityRole="button" accessibilityLabel="Adicionar valor à reserva de emergência"><Ionicons accessibilityElementsHidden name="add" size={16} color="#fff" /><Text style={styles.addReserveBtnText}>Guardar</Text></TouchableOpacity>
          </View>
          <View style={styles.goalRow}>
            ...
            <TouchableOpacity style={styles.iconButton} onPress={() => { setNewGoal(safeGoal.toFixed(2).replace('.', ',')); setGoalModalVisible(true); }} accessibilityRole="button" accessibilityLabel="Editar meta da reserva de emergência"><Ionicons accessibilityElementsHidden name="create-outline" size={20} color="#6366f1" /></TouchableOpacity>
          </View>
      ...
      <Modal visible={modalVisible} animationType="slide" transparent onRequestClose={closeReserveModal}>
        <KeyboardAvoidingView style={styles.modalOverlay} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View style={styles.modalContainer} accessibilityViewIsModal>
            ...
            <View style={styles.inputWrapper}><Text style={styles.currencyPrefix}>R$</Text><TextInput style={styles.modalInputAmount} keyboardType="numeric" placeholder="0,00" value={reserveAmount} onChangeText={handleAmountChange} editable={!submittingReserve && !intentLocked} accessibilityLabel="Valor a guardar na reserva de emergência" /></View>
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity style={[styles.saveBtn, submittingReserve && styles.buttonDisabled]} onPress={handleSaveReserve} disabled={submittingReserve} accessibilityRole="button" accessibilityLabel="Confirmar valor da reserva" accessibilityHint={intentLocked ? 'Repete a mesma intenção da reserva com o valor original' : 'Adiciona o valor à reserva'} accessibilityState={{ disabled: submittingReserve, busy: submittingReserve }}>{submittingReserve ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveBtnText}>{intentLocked ? 'Tentar mesma intenção' : 'Confirmar'}</Text>}</TouchableOpacity>
            </View>
          </View>
        </KeyboardAvoidingView>
      </Modal>
      ...
      <Modal visible={goalModalVisible} animationType="slide" transparent onRequestClose={() => !submittingGoal && setGoalModalVisible(false)}>
        <KeyboardAvoidingView style={styles.modalOverlay} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View style={styles.modalContainer} accessibilityViewIsModal>
            ...
            <View style={styles.inputWrapper}><Text style={styles.currencyPrefix}>R$</Text><TextInput style={styles.modalInputAmount} keyboardType="numeric" value={newGoal} onChangeText={handleGoalAmountChange} accessibilityLabel="Nova meta total da reserva de emergência" /></View>
            <View style={styles.modalActions}><TouchableOpacity style={styles.cancelBtn} onPress={() => setGoalModalVisible(false)} disabled={submittingGoal}><Text style={styles.cancelBtnText}>Cancelar</Text></TouchableOpacity><TouchableOpacity style={styles.saveBtn} onPress={handleSaveGoal} disabled={submittingGoal}>{submittingGoal ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveBtnText}>Salvar meta</Text>}</TouchableOpacity></View>
          </View>
        </KeyboardAvoidingView>
      </Modal>
    </View>
